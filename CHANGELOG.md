...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Introduced new `locals.tf` at the project root holding the resource-dependent outputs (`aws_account_id`, `aws_caller_arn`, `aws_caller_user`, `ec2_ssh_key`, `aws_ec2_private_ip`, `aws_ec2_public_ip`) as the two-layer-defense pattern (`var.use_mocks ? <mock_literal> : try(<production_expression>, <fallback_literal>)`). Refactored the corresponding `output` blocks in `012_outputs.tf` to be thin passthroughs (`value = local.<name>`). Production behaviour is unchanged — the locals' production branch evaluates identically to the previous in-output expression. The refactor lets `terraform console` evaluate every output without real AWS state, which the parallel test precompute (forthcoming) consumes to populate per-scenario `outputs.json` files. Mock-mode literals: `aws_account_id`/`aws_caller_arn`/`aws_caller_user`/`ec2_ssh_key` → `"N/A"`; `aws_ec2_private_ip` → `"192.168.0.1"`; `aws_ec2_public_ip` → `"1.2.3.4"`.
            ...
            - Migrated container image paths in `assets/src/docker_compose/docker-compose.yml.tpl` from the legacy `cr.seqera.io/private/nf-tower-enterprise/*` namespace to Harbor's current `cr.seqera.io/enterprise/*` namespace. Image content and version tags are unchanged (same manifest digests). Affects Platform (`platform/migrate-db`, `platform/backend` — used by both the `backend` and `cron` services, `platform/frontend`), Studios (`studios/proxy`, `studios/server`), Wave-Lite (`wave/server`), and Groundswell (`groundswell/server`). Deployers logged into `cr.seqera.io` need read access on the new `enterprise/*` projects in Harbor. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)
            - Bumped container image tags in `templates/TEMPLATE_terraform.tfvars` to align with the Seqera Platform v26.1.x release set ([`#332`](https://github.com/seqeralabs/cx-field-tools-installer/issues/332)): `tower_container_version` → `v26.1.2`, `swell_container_version` → `0.4.15`, `data_studio_container_version` → `0.12.1`, `wave_lite_container_version` → `v1.33.0`. The `data_studio_options` map was refreshed accordingly: `0.9.0` images removed, `0.11.0` images retained but marked `"deprecated"`, and new `0.12.1` images added as the `"recommended"` set across VSCode, Jupyter, RIDE, and Xpra. Note the Xpra qualifier for `0.12.1` is `XPRA-6-2-0-R2-1-0-12-1` (preserving the `.0` patch digit from xpra `6.2.0-r2`), which is the full hyphenization — distinct from the abbreviated `XPRA-6-2-R2-1-0-11-0` form used for the `0.11.0` sibling. The qualifier is emitted as a key prefix in `tower.env` (`TOWER_DATA_STUDIO_TEMPLATES_<qualifier>_*`); deployers consuming those keys should reference the new form.
            - Added optional Data Lineage support ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)). Two new tfvars knobs: `flag_enable_data_lineage` (master gate, default `false`) and `data_lineage_allowed_workspaces` (CSV workspace allowlist; empty = all workspaces). When enabled, `tower.env` renders `TOWER_LINEAGE_ALLOWED_WORKSPACES` and the installer attaches a new IAM policy (`${global_prefix}_policy_lineage`) to the EC2 instance role granting S3 + SQS actions scoped to `seqera-lineage-*` resource ARNs only; off-state renders a commented `# TOWER_LINEAGE_NOT_ENABLED` marker. `check_configuration.py` enforces a Platform v26.1.0+ floor and warns when lineage is enabled alongside `flag_iam_use_prexisting_role_arn = true` (same warning was retrofitted onto the analogous SES + pre-existing-role case). Per [Design Decision #19](documentation/design_decisions.md), the installer does not provision the SQS queue or S3 bucket — Platform creates these per-workspace under the `seqera-lineage-*` prefix when a workspace enables lineage via the UI.
            - Added 15 new Studios environment variables introduced in Platform v26.1 and connect-proxy v0.11.1 ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)). All wired through `terraform.tfvars` → `variables.tf` → templates. New variables by group: **General** — `TOWER_DATA_STUDIO_DEFAULT_LIFESPAN` (default `8`), `TOWER_DATA_STUDIO_LIST_MAX_ALLOWED` (default `100`), `TOWER_DATA_STUDIO_PRIVATE_STUDIO_BY_DEFAULT` (default `false`), `TOWER_DATA_STUDIO_FEATURE_MANIFEST_URL`, `TOWER_DATA_STUDIO_CONNECT_IFRAME_ALLOWED_WORKSPACES`; **SSH** — `TOWER_SSH_KEYS_SUPPORTED_TYPES` (default: five key type CSV); **Metrics** — `TOWER_STUDIO_METRICS_ENABLED_WORKSPACES`, `TOWER_STUDIO_METRICS_RETENTION_DAYS` (default `90`); **Wave integration** — `TOWER_DATA_STUDIO_WAVE_DISALLOWED_REGISTRIES`, `TOWER_DATA_STUDIO_WAVE_CUSTOM_IMAGE_REGISTRY`, `TOWER_DATA_STUDIO_WAVE_CUSTOM_IMAGE_REPOSITORY`, `TOWER_DATA_STUDIO_WAVE_CUSTOM_IMAGE_NAME_STRATEGY` (default `tagPrefix`), `TOWER_DATA_STUDIO_WAVE_STATUS_CHECK_INITIAL_DELAY` (default `5s`), `TOWER_DATA_STUDIO_WAVE_STATUS_CHECK_RATE` (default `30s`); **Connect proxy SSH** — `CONNECT_SSH_KEY_VALUE_BASE64` (base64 fallback for `CONNECT_SSH_KEY_PATH`). Wave vars are only rendered when `flag_use_wave = true`; optional vars with no default are omitted when empty.
            - Added full Connect proxy environment variable coverage to `data-studios.env` ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)). 17 new `terraform.tfvars` variables plus 2 SSM-sourced credentials (`CONNECT_REDIS_USER`, `CONNECT_REDIS_PASSWORD`) cover all entries documented at the [connect environment variables reference](https://docs.seqera.io/platform-enterprise/enterprise/install-studios#connect-environment-variables). New variables by group: **Server config** — `connect_listener_port` (default `7777`), `connect_tunnel_port` (default `7070`), `connect_storage_root` (default `/data`), `connect_host_domain`, `connect_management_port`, `connect_management_auth_key`, `connect_log_level` (default `debug`); **OIDC/client** — `connect_client_name` (default `tower-connect-proxy-client`), `connect_grant_type` (default `authorization_code`); **Redis extras** — `connect_redis_prefix` (default `connect:session`), `connect_redis_tls_enable` (default `false`), `connect_redis_tls_skip_verify` (default `false`), `connect_redis_tls_key_file`, `connect_redis_tls_cert_file`; **SSH tuning** — `connect_ssh_max_connections` (default `2000`), `connect_ssh_max_conn_channels` (default `30`), `connect_ssh_handshake_timeout` (default `1m`). Redis credentials are optional and injected at deploy time from SSM (`/config/<app_name>/connect/redis/user` and `/config/<app_name>/connect/redis/password`) only when Studios is enabled and the SSM value is non-empty; set them only if Redis authentication is required. Optional vars without a set value render a commented `_NOT_SET=DO_NOT_UNCOMMENT` marker. `CONNECT_LOG_LEVEL` was previously hardcoded to `debug` in `data-studios.env`; it now reflects the `connect_log_level` tfvars value.
            - TBD
        <br /><br />
        ...
        - Documentation
            ...
            - Added 17 Connect proxy variables to `TEMPLATE_terraform.tfvars` with inline comments and section-level links to the [connect environment variables reference](https://docs.seqera.io/platform-enterprise/enterprise/install-studios#connect-environment-variables). [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)
            - TBD
        <br /><br />
        ...
        - Testing
            ...
            - Templatefile test path no longer requires `terraform plan` (see [`#353`](https://github.com/seqeralabs/cx-field-tools-installer/issues/353)). `module.connection_strings.*` outputs are now resolved via a single `terraform console` call that's ~1-2 s vs the ~30+ s the equivalent plan cost. Plan path remains available as an opt-in for tests that genuinely need plan-derived state (`test_outputs.py`, `test_variable_validation.py`); templatefile-only tests (`test_config_file_content.py`, `test_ansible_files.py`, `test_testcontainer_execution.py`) migrated to the new `stage_tfvars(...)` + `generate_tc_files(None, ...)` shape. New regression test `tests/unit/framework/test_console_locals_resolvability.py` walks every `local.<name>` referenced from `009_define_file_templates.tf` and asserts each resolves via `terraform console`, catching the silent-corruption failure mode that would otherwise occur if a future local mixed tfvars-known and resource-attribute branches in a ternary. Documented the two-layer defense pattern (`var.use_mocks ? null : try(<ref>, null)`) above the `module "connection_strings"` invocation in [`000_main.tf`](000_main.tf) so future contributors apply the same shape to new resource-attribute inputs.
            - Phase 2 of the `hcl2json` Docker-overhead removal (linux/arm64): expanded `_is_supported_extraction_platform()`, the `extract_hcl2json` Makefile recipe, and the smoke-test skipif to accept `aarch64` and `arm64` machine names in addition to `x86_64`. Removed the hardcoded `--platform linux/amd64` flag from the `docker run` fallback so the daemon can pick the matching architecture from the multi-arch manifest natively (forcing amd64 would silently run under emulation on arm64). Republished the vendored container as a multi-arch manifest at `ghcr.io/seqeralabs/cx-field-tools-installer/hcl2json:0.6-vendored-multiarch` (mirror of upstream `tmccombs/hcl2json:0.6`, full blob copy via `skopeo copy --multi-arch all` — no runtime dependency on Docker Hub); the original `:vendored` tag stays pointing at the prior amd64-only image for reverse compatibility. The `extract_hcl2json` recipe also now probes `docker info` before attempting extraction: if the daemon is unreachable but a binary already exists at `HCL2JSON_BIN` (e.g. inside a sandbox that mounts the directory but blocks Docker), the recipe degrades gracefully and reuses the existing binary; if neither is available, it fails fast with an actionable message instead of the cryptic `docker create` / `docker cp` error chain that previously surfaced. [`#352`](https://github.com/seqeralabs/cx-field-tools-installer/issues/352)
            - Batched per-template `terraform console` renders into a single call per scenario ([`#361`](https://github.com/seqeralabs/cx-field-tools-installer/issues/361)). `tests/utils/terraform/template_generator.py:generate_tc_files` now collects every cache-miss templatefile payload into one `jsonencode({...})` map and renders the whole batch via a single `terraform console` invocation (saving the ~1.85 s startup that was previously paid per template). Cold-cache `make run_tests_core_only` drops from ~155 s to ~87 s (~44 % reduction, ~68 s saved) on top of the #353 win. The single-template `write_populated_templatefile` helper is retained for unit-level rendering and for narrowing blame when a batched render fails; the `_is_wave_lite_rds` SQL carve-out continues through `sedalternative.py` unchanged. Documented tradeoff: a failure in any single payload fails the whole batch — `terraform console`'s native error identifies the offending expression, but the pytest traceback points at `render_templatefiles_batched` rather than the failing `desired_files` entry.
...
||||
| New | Data Lineage | `flag_enable_data_lineage` | Master gate for Nextflow Data Lineage (Platform v26.1.0+). When `true`, `tower.env` renders `TOWER_LINEAGE_ALLOWED_WORKSPACES` and the installer attaches an IAM policy granting S3/SQS actions scoped to `seqera-lineage-*` resource ARNs. Defaults to `false`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Data Lineage | `data_lineage_allowed_workspaces` | Comma-separated list of numeric workspace IDs allowed to use Data Lineage. Empty string = all workspaces (when `flag_enable_data_lineage = true`). Ignored when the master gate is off. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
||||
...
| Modified | Studios | `data_studio_container_version` | Updated from 0.9.0 to 0.12.1. Note: Studios SSH requires connect-proxy >= 0.10.0. [`#332`](https://github.com/seqeralabs/cx-field-tools-installer/issues/332) |
| Modified | Studios | `data_studio_options` | Removed 0.9.0 images. Added 0.11.0 (deprecated) and 0.12.1 (recommended) for VSCode, Jupyter, RIDE, and Xpra. Xpra 0.12.1 qualifier uses the full `XPRA-6-2-0-R2-1-0-12-1` hyphenization. [`#332`](https://github.com/seqeralabs/cx-field-tools-installer/issues/332) |
...
||||
| New | Connect proxy | `connect_listener_port` | Port connect-server listens on for incoming connections. Default `7777`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_tunnel_port` | Port used to open new tunnels. Default `7070`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_storage_root` | Root directory for connect proxy data. Default `/data`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_host_domain` | Host domain suffix for the server. Leave empty to use the default. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_management_port` | Port for metrics, readiness, and shutdown endpoints. Leave empty to disable. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_management_auth_key` | Authentication key protecting management service endpoints. Leave empty if not using the management port. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_log_level` | Logging verbosity (`debug`, `info`, `warn`, `error`). Default `debug`. Previously hardcoded to `debug` in `data-studios.env`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_client_name` | OIDC client identifier for the Studio provider. Default `tower-connect-proxy-client`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_grant_type` | OAuth grant type for the Studio provider. Default `authorization_code`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_redis_prefix` | Prefix for tunnel keys in Redis. Default `connect:session`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_redis_tls_enable` | Enable TLS for the Redis connection. Default `false`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_redis_tls_skip_verify` | Skip TLS certificate verification. Only applies when `connect_redis_tls_enable = true`. Default `false`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_redis_tls_key_file` | Path to the TLS key file for Redis. Leave empty if not using TLS. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_redis_tls_cert_file` | Path to the TLS certificate file for Redis. Leave empty if not using TLS. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_ssh_max_connections` | Maximum concurrent SSH connections allowed. Default `2000`. Only rendered when `flag_enable_data_studio_ssh = true`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_ssh_max_conn_channels` | Maximum concurrent channels per SSH client connection. Default `30`. Only rendered when `flag_enable_data_studio_ssh = true`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
| New | Connect proxy | `connect_ssh_handshake_timeout` | SSH handshake timeout duration. Default `1m`. Only rendered when `flag_enable_data_studio_ssh = true`. [`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378) |
||||
