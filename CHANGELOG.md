...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Added Compute Environment Cleanup support ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)). Seven new tfvars knobs: `tower_compute_env_cleanup_enabled` (master gate, default `false`) and `tower_compute_env_cleanup_{delay,interval,batch_size,time_offset,stuck_creating_timeout,stuck_deleting_timeout}` (operational tuning, defaults match Platform's built-in cron-profile values: `1m`, `1h`, `10`, `60s`, `1h`, `1h`). When enabled, `tower.env` renders all 7 `TOWER_COMPUTE_ENV_CLEANUP_*` vars; the Platform cron service picks these up via its `application-cron.yml` profile and runs a periodic job that identifies and deletes compute environments stuck in `CREATING` or `DELETING` states. Off-state renders a commented `# TOWER_COMPUTE_ENV_CLEANUP_NOT_ENABLED` marker. Requires Platform >= v26.1.0.
