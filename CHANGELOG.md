...
- **Notable Changes**
    - **CX Installer**
        ...
        - Testing
            ...
            - Phase 2 of the `hcl2json` Docker-overhead removal (linux/arm64): expanded `_is_supported_extraction_platform()`, the `extract_hcl2json` Makefile recipe, and the smoke-test skipif to accept `aarch64` and `arm64` machine names in addition to `x86_64`. Removed the hardcoded `--platform linux/amd64` flag from the `docker run` fallback so the daemon can pick the matching architecture from the multi-arch manifest natively (forcing amd64 would silently run under emulation on arm64). **Activation requires republishing the vendored container as a multi-arch manifest** (mirror of upstream `tmccombs/hcl2json` via `docker buildx imagetools create`) and updating the pinned digest in this repo — marked with `TODO(#352-phase2)` in [Makefile](Makefile) and [scripts/installer/utils/extractors.py](scripts/installer/utils/extractors.py). [`#352`](https://github.com/seqeralabs/cx-field-tools-installer/issues/352)
            - TBD
