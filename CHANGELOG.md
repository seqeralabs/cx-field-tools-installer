...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Re-monolithised the mid-pipeline VM-configuration stages in `011_configure_vm.tf`. The previous chain of 8 sequential `null_resource` blocks (host_configuration → ansible_setup → system_packages → update_configuration_files → pull_containers_run_tower → wait_for_tower → patch_groundswell → run_seqerkit) is now a single `null_resource` that delegates to `assets/src/bash/remote/orchestrate.sh` on the host. The orchestrator wraps each stage in bracketed log markers (`STAGE START` / `STAGE OK` / `STAGE FAILED`) so per-stage failure attribution survives — search the on-host log for `STAGE FAILED` to find the failure point. Persistent log location: `/home/ec2-user/tower-installer-logs/apply-<UTC>.log`. The two remaining `null_resource` blocks (`ssh_connectivity_check`, `file_transfer`) stay separate as they're local-side prerequisites. Net plan output: 3 null_resource lines per apply instead of 10. **Drift defence is unchanged** — `always_run = timestamp()` on the new `configure_vm` resource still triggers a full Ansible reconcile every apply. [`#410`](https://github.com/seqeralabs/cx-field-tools-installer/issues/410)
        <br /><br />
