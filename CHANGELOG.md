...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Migrated 11 single-variable validation checks from `scripts/installer/validation/check_configuration.py` to native Terraform `validation` blocks in `variables.tf` — `tower_server_url`, `tower_root_users`, `tower_db_url`, `tower_db_driver`, `tower_db_dialect`, `db_engine_version`, `db_container_engine_version`, `tower_container_version`, `data_studio_eligible_workspaces`, `data_studio_ssh_eligible_workspaces`, `pipeline_versioning_eligible_workspaces`. Errors now fire at plan time (earlier than the pre-plan Python script) and are co-located with each variable's declaration. The three workspace-ID variables use empty-string-OR-numeric-CSV semantics, so they are validated regardless of whether the related feature flag is set. Cross-variable and warning-only checks remain in the Python script.
            - TBD
        <br /><br />
