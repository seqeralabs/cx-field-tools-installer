...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Added Compute Environment Cleanup support as a single object `tower_compute_env_cleanup` ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378), [PR #387](https://github.com/seqeralabs/cx-field-tools-installer/pull/387)). Seven `optional()` sub-fields: `enabled` (bool, default `false`), `delay` (default `"1m"`), `interval` (default `"1h"`), `batch_size` (number, default `10`), `time_offset` (default `"60s"`), `stuck_creating_timeout` (default `"1h"`), `stuck_deleting_timeout` (default `"1h"`). Five validation blocks enforce a duration regex on the duration-shaped fields. When `enabled = true`, `tower.env` renders the seven `TOWER_COMPUTE_ENV_CLEANUP_*` env vars; the Platform cron service picks them up via its `application-cron.yml` profile to run a periodic job that identifies and deletes compute environments stuck in `CREATING` or `DELETING` states. Off-state renders a commented `# TOWER_COMPUTE_ENV_CLEANUP_NOT_ENABLED` marker. Requires Platform v26.1.0+; pre-v26.1 instances silently ignore the env vars, but `check_configuration.py:verify_compute_env_cleanup_platform_version` warns when the feature is opted in on a pre-v26.1 deployment.
            - Added Audit Log v2 and cron cleanup support as a single nested object `tower_audit_log_v2` ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378), [PR #389](https://github.com/seqeralabs/cx-field-tools-installer/pull/389)). The object exposes three top-level fields (`write_mode` default `"dual"`, `csv_export_max_logs` default `500000`, `pre_post_change_enabled` default `false`) plus a nested `cleanup` sub-object (`enabled` default `true`, `interval` default `"5m"`, `delay` default `"10s"`, `chunk_size` default `1000`). All sub-fields are `optional()` with sensible defaults so deployers can omit the entire block or override a single sub-field without restating the whole object. Validation blocks enforce `write_mode ∈ {"v1", "v2", "dual"}` and a duration regex on `cleanup.interval`/`cleanup.delay`. `tower.env.tpl` direct-interpolates the values (no per-flag if/else) and wraps the four cron sub-vars in a single `%{ if cleanup.enabled }` gate — disabling cleanup emits `TOWER_CRON_AUDIT_LOG_CLEAN_UP_ENABLED=false` and skips the interval/delay/chunk_size lines entirely. Requires Platform v26.1.0+; pre-v26.1 instances silently ignore the emitted env vars, but `check_configuration.py:verify_audit_log_v2_platform_version` emits a warning to flag the version mismatch.
            ...
            - Announced **seqerakit deprecation**. The `seqerakit_*` tfvars, the post-deployment seqerakit step, and the generated `seqerakit.yml` template remain in the repo to avoid breaking in-flight deployments but are **no longer actively maintained** and not guaranteed to work out of the box. They will be replaced by the upcoming [Seqera Terraform provider](https://registry.terraform.io/providers/seqeralabs/seqera/latest/docs). Deprecation does NOT remove the SSM bootstrap requirement: the `secrets_bootstrap_seqerakit` SSM path still must exist (the installer reads it at plan time) but its contents may be placeholder values when `flag_run_seqerakit = false`. The one exception is the `TOWER_AWS_ROLE` key inside that secret, which the EC2 instance-credentials flow (`flag_allow_aws_instance_credentials`) continues to read until the Terraform provider takes over.
            - Replaced the `get-pip.py` network bootstrap in `assets/src/aws/launch_template_ec2.tpl` with `yum install -y python3-pip`. Upstream `get-pip.py` now requires Python ≥ 3.10, but Amazon Linux 2023 ships with Python 3.9 as the system default — the bootstrap was aborting on that interpreter. The replacement removes the network round-trip to `bootstrap.pypa.io`, drops the bespoke curl/retry loop, and pulls pip from the signature-verified Amazon Linux repo (`python3-pip 21.3.1-2.amzn2023.0.19`). Drive-by hardening on the same script: added `set -euo pipefail` for fail-fast semantics, and changed `yum update` to `yum update -y` to avoid hanging on the unattended-confirmation prompt.
            - TBD
        <br /><br />
...
| Modified | Studios | `data_studio_container_version` | Updated from 0.9.0 to 0.11.0 Note: Studios SSH requires connect-proxy >= 0.10.0. [`#332`](https://github.com/seqeralabs/cx-field-tools-installer/issues/332) |
