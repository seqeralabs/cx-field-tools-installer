...
- **Notable Changes**
    - **CX Installer**
        - General
            ...
            - Added Audit Log v2 and cron cleanup support ([`#378`](https://github.com/seqeralabs/cx-field-tools-installer/issues/378)). Seven new tfvars knobs wired unconditionally into `tower.env`: three for Audit Log v2 (`tower_audit_log_v2_write_mode` default `"dual"`, `tower_audit_log_v2_csv_export_max_logs` default `"500000"`, `tower_audit_log_v2_pre_post_change_enabled` default `false`) and four for the cron audit log cleanup job (`tower_cron_audit_log_clean_up_enabled` default `true`, `tower_cron_audit_log_clean_up_interval` default `"5m"`, `tower_cron_audit_log_clean_up_delay` default `"10s"`, `tower_cron_audit_log_clean_up_chunk_size` default `"1000"`). All vars render on every apply regardless of other feature flags. The cleanup vars complement the existing `tower_audit_retention_days` (YAML-based time-offset) — they control the cron job's scheduling and batch behaviour via env var form. Requires Platform >= v26.1.0.
            - TBD
        <br /><br />
