...
if SEQERAKIT_USE_HOSTS_FILE == "true":
    TOWER_API_ENDPOINT = "http:/localhost:8000/api"
else:
    ...
    for line in lines:
        if line.startswith("TOWER_ROOT_USERS="):
            ...
            try:
                ...
                raise Exception(
                    f"Sorry, your `tower.env` has an unusable TOWER_ROOT_USERS value. Error: {e}"
                )

    if EMAIL_ADDRESS == "":
        raise Exception(
            "Sorry, your `tower.env` has an empty TOWER_ROOT_USERS value. Please populate and try again."
        )
...
tower_db_user = subprocess.run(
    ...
    + SSM_TOWER_DB_USER
    + " --with-decryption --query 'Parameters[*].{Value:Value}' --output text",
    shell=True,
    text=True,
    capture_output=True,
)
...
login_command = (
    f"curl -Ss '{TOWER_API_ENDPOINT}/gate/access' -H 'Content-Type: application/json' -d '"
    + login_payload
    + "' -o /dev/null"
)
...
if str(is_external_db_in_use) == "true":
    ...
    # Old call was too brittle -- assumed the first root user would be the first DB entry.
    # # Only true for first time greenfield deployments.
...
pat_command = (
    f"curl -Ss '{TOWER_API_ENDPOINT}/tokens' -H 'Content-Type: application/json' -H 'Authorization: Bearer {jwt_token.stdout.strip()}' -d '{pat_payload}' | jq -r '.accessKey'",
)
...
subprocess.run(
    [
        "echo 'export TOWER_ACCESS_TOKEN="
        + pat_token.stdout
