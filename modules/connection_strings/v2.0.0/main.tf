locals {
  ...
  # TOWER CORE
  # ---------------------------------------------------------------------------------------
  # - If insecure, use "http://" and set port.
  # - If secure, "use https://" and dont set port.
  tower_base_url        = var.tower_server_url
  platform_url_secure   = "https://${var.tower_server_url}"
  platform_url_insecure = "http://${var.tower_server_url}:8000"
  tower_server_url      = var.flag_do_not_use_https ? local.platform_url_insecure : local.platform_url_secure
  tower_api_endpoint    = "${local.tower_server_url}/api"

  # Only one DB URL should ever be populated. Smash everything together in a unified catch-all.
  # IDEA: Consider using different port for extra testing?
  # NOTE! Leading "?" already in connection string value!
  ...
  swell_db_dns = local.groundswell_enabled ? local.platform_db_dns : "N/A"
