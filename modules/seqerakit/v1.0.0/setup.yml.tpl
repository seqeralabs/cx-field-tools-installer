...
%{~ if seqerakit_flag_credential_create_aws == true }
  - type: 'aws'
    ...
    workspace: "${seqerakit_org_name}/${seqerakit_workspace_name}"
    access-key: ${aws_access_key_env_var}
    secret-key: ${aws_secret_key_env_var}
    %{~ if seqerakit_flag_credential_use_aws_role == true ~}
    assume-role-arn: ${aws_role_arn_env_var}
    %{~ endif ~}
    overwrite: False
%{ endif ~}
...
%{~ if seqerakit_flag_credential_create_docker == true }
  - type: 'container-reg'
    ...
    workspace: "${seqerakit_org_name}/${seqerakit_workspace_name}"
    username: ${docker_username_env_var}
    password: ${docker_password_env_var}
    registry: 'docker.io'
    overwrite: False
%{ endif ~}
...
%{~ if seqerakit_flag_credential_create_github == true }
  - type: 'github'
    ...
    workspace: "${seqerakit_org_name}/${seqerakit_workspace_name}"
    username: ${github_username_env_var}
    password: ${github_token_env_var}
    overwrite: False
%{ endif ~}

compute-envs:
%{~ if seqerakit_aws_use_forge == true }
  - type: aws-batch
    config-mode: forge
