...
locals {
  ...
  # Housekeeping 
  # ---------------------------------------------------------------------------------------
  global_prefix = var.flag_use_custom_resource_naming_prefix ? var.custom_resource_naming_prefix : "tf-${var.app_name}-${random_pet.stackname.id}"


  # Networking
  # ---------------------------------------------------------------------------------------
  vpc_id                      = var.flag_create_new_vpc ? module.vpc[0].vpc_id : var.vpc_existing_id
  vpc_private_route_table_ids = var.flag_create_new_vpc ? module.vpc[0].private_route_table_ids : data.aws_route_tables.preexisting[0].ids
  ...
  dns_create_alb_record = var.flag_create_load_balancer && !var.flag_create_hosts_file_entry
  dns_create_ec2_record = !var.flag_create_load_balancer && !var.flag_create_hosts_file_entry

  dns_zone_id = (
    var.flag_create_route53_private_zone ? aws_route53_zone.private[0].id :
    var.flag_use_existing_route53_public_zone ? data.aws_route53_zone.public[0].id :
    var.flag_use_existing_route53_private_zone ? data.aws_route53_zone.private[0].id :
    "No_Match_Found"
  )

  dns_instance_ip = (
    var.flag_make_instance_private ? aws_instance.ec2.private_ip :
    var.flag_make_instance_private_behind_public_alb ? aws_instance.ec2.private_ip :
    var.flag_private_tower_without_eice ? aws_instance.ec2.private_ip :
    var.flag_make_instance_public ? aws_eip.towerhost[0].public_ip :
    "No_Match_Found"
  )
  ...
  alb_public_access = (
    var.flag_make_instance_public || var.flag_make_instance_private_behind_public_alb
    ? var.sg_ingress_cidrs
    : []
  )

  alb_private_new = (
    var.flag_make_instance_private && var.flag_create_new_vpc
    ? [var.vpc_new_cidr_range]
    : []
  )

  alb_private_existing = (
    var.flag_make_instance_private && var.flag_use_existing_vpc
    ? [data.aws_vpc.preexisting[0].cidr_block]
    : []
  )

  alb_private_no_eice = (
    var.flag_private_tower_without_eice && var.flag_use_existing_vpc
    ? [data.aws_vpc.preexisting[0].cidr_block]
    : []
  )
  ...
  # alb_ingress_cidrs = (
  #   var.flag_make_instance_public  || var.flag_make_instance_private_behind_public_alb  ? var.sg_ingress_cidrs :
  #   var.flag_make_instance_private  && var.flag_create_new_vpc  ? distinct(concat([var.vpc_new_cidr_range], var.sg_ingress_cidrs)):
  #   var.flag_make_instance_private  && var.flag_use_existing_vpc  ? distinct(concat([data.aws_vpc.preexisting.cidr_block], var.sg_ingress_cidrs)) :
  #   var.flag_private_tower_without_eice  && var.flag_use_existing_vpc  ? distinct(concat([data.aws_vpc.preexisting.cidr_block], var.sg_ingress_cidrs)) :
  #   ["No CIDR block found"]
  # )
  ...
  populate_external_db = var.flag_create_external_db || var.flag_use_existing_external_db


  # Docker-Compose
  # ---------------------------------------------------------------------------------------
  docker_compose_file = (
    var.flag_use_custom_docker_compose_file ? "dc_custom.yml" :
    var.flag_use_container_db ? "dc_with_db.yml" :
    "dc_without_db.yml"
  )
  ...
  oidc_auth         = var.flag_oidc_use_generic ? "auth-oidc" : ""
  oidc_google       = var.flag_oidc_use_google ? "auth-google" : ""
  oidc_github       = var.flag_oidc_use_github ? "auth-github" : ""
  oidc_consolidated = join(",", compact([local.oidc_auth, local.oidc_google, local.oidc_github]))
  ...
  studio_uses_distroless = (
    length(regexall("^0\\.7\\.[8-9]", var.data_studio_container_version)) > 0 ||
    length(regexall("^0\\.8\\.[0-9]", var.data_studio_container_version)) > 0
  )
  ...
  # Wave
  # ---------------------------------------------------------------------------------------
  wave_enabled              = var.flag_use_wave || var.flag_use_wave_lite
  wave_lite_redis_container = !(var.flag_use_wave_lite && var.flag_create_external_redis)
  wave_lite_db_container    = !(var.flag_use_wave_lite && var.flag_create_external_db)
  ...
  flag_new_enough_for_migrate_db = (
    length(regexall("^v23\\.4\\.[1-9]", var.tower_container_version)) > 0 ||
    length(regexall("^v2[4-9]", var.tower_container_version)) > 0
  )

  # Account for changes in tower.yml due to Micronaut 4
  flag_using_micronaut_4 = (
    length(regexall("^v24\\.[0-9]", var.tower_container_version)) > 0 ||
    length(regexall("^v2[5-9]", var.tower_container_version)) > 0
  )
}
...
module "connection_strings" {
  ...
  # flag_use_wave                 = var.flag_use_wave ?? Should this be lite ? 
  flag_use_wave_lite            = var.flag_use_wave_lite

  # Tower Configuration
  tower_server_url = var.tower_server_url
  tower_db_url     = var.flag_use_existing_external_db ? var.tower_db_url : ""
