...
2. [`ssm_sensitive_values_tower.json`](templates/TEMPLATE_ssm_sensitive_values_tower.json)<br />Used to define secrets like credentials and passwords which the Seqera Platform needs to fulfill its duties.

   This file copied exists to make SSM data population more convenient and **should not** be stored in source-control. Copy and rename this file to `ssm_sensitive_values_tower.json` and configure appropriately.

3. [`ssm_sensitive_values_seqerakit.json`](templates/TEMPLATE_ssm_sensitive_values_seqerakit.json) (Optional) <br />Used to define secrets like tokens and credentials which the [Seqerakit](https://github.com/seqeralabs/seqera-kit) tool needs to fulfill its duties.

   This file copied exists to make SSM data population more convenient and **should not** be stored in source-control. Copy and rename this file to `ssm_sensitive_values_seqerakit.json` and configure appropriately.

4. [`ssm_sensitive_values_groundswell.json`](templates/TEMPLATE_ssm_sensitive_values_groundswell.json) (Optional) <br />Used to define credentials for the Groundswell optimization service. Copy and rename this file to `ssm_sensitive_values_groundswell.json` and configure appropriately.

   This copied file exists to make SSM data population more convenient and **should not** be stored in source-control.
