...
def verify_only_one_true_set(data: SimpleNamespace):
    ...
    only_one_true_set([ data.flag_create_external_db, data.flag_use_existing_external_db, data.flag_use_container_db ])
    only_one_true_set([ data.flag_create_external_redis, data.flag_use_container_redis ])
    only_one_true_set([ data.flag_create_load_balancer, data.flag_generate_private_cacert, data.flag_use_existing_private_cacert, data.flag_do_not_use_https ])
...
def verify_tfvars_config_dependencies(data: SimpleNamespace):
    ...
    ensure_dependency_populated(data.flag_use_existing_route53_private_zone, data.existing_route53_private_zone_name, '`existing_route53_private_zone_name` value is missing.')
...
def verify_tower_server_url(data: SimpleNamespace):
    ...
    if data.tower_server_port != "8000":
        logger.warning("Your Tower instance is using a non-default port (8000). Ensure your Docker-Compose file is updated accordingly.")
...
def verify_tower_self_signed_certs(data: SimpleNamespace):
    ...
    if data.flag_generate_private_cacert:
        if not data.bucket_prefix_for_new_private_ca_cert.startswith('s3://'):
            log_error_and_exit(' Field `bucket_prefix_for_new_private_ca_cert` must start with `s3://`')
...
def verify_subnet_privacy(data: SimpleNamespace):
    ...
    if data.flag_create_new_vpc:
        if data.flag_make_instance_public:
            subnet_privacy(data.vpc_new_ec2_subnets, public_subnets, "`vpc_new_ec2_subnets` must contain public subnets.")
        else:
            subnet_privacy(data.vpc_new_ec2_subnets, private_subnets, "`vpc_new_ec2_subnets` must contain private subnets.")
 
    if data.flag_use_existing_vpc:
        if data.flag_make_instance_public:
            subnet_privacy(data.vpc_existing_ec2_subnets, public_subnets, "`vpc_existing_ec2_subnets` must contain public subnets.")
        else:
            subnet_privacy(data.vpc_existing_ec2_subnets, private_subnets, "`vpc_existing_ec2_subnets` must contain private subnets.")
    ...
    if data.flag_create_load_balancer:
        if data.flag_create_new_vpc:
            subnet_privacy(data.vpc_new_alb_subnets, public_subnets, "`vpc_new_alb_subnets` must contain public subnets.")
        else:
...
def verify_ingress_and_egress(data: SimpleNamespace, data_dictionary: dict):
    ...
    egress_sgs = ["sg_egress_eice", "sg_egress_tower_ec2", "sg_egress_tower_alb", "sg_egress_batch_ec2", "sg_egress_interface_endpoint"]
...
def verify_ami_update_behaviour(data: SimpleNamespace):
    ...
    if (data.ec2_update_ami_if_available):
        logger.info("Your EC2 AMI will update as newer images are available. This means your VM will occasionally be destroyed and recreated.")

        if (data.flag_use_container_db):
            logger.warning("Your docker db container will destroyed when you EC2 AMI is updated. Ensure this fits your intention.")
...
def verify_database_configuration(data: SimpleNamespace):
    ...
    if data.flag_use_container_db:
        ...
        if data.tower_db_url != "db:3306":
            logger.warning("You are using a non-standard db container name or port. Ensure Docker-Compose config is updated accordingly.")

    if ( data.flag_use_existing_external_db ):
        logger.warning("You are using a pre-existing external database. Please ensure you create the database, user, and append the database name to `tower_db_url`.")

    if data.db_deletion_protection:
        logger.info("You have Deletion Protection enabled for your external DB. This will affect easy teardown during testing.")
    ...
    if data.skip_final_snapshot:
        logger.warning("You have disabled a final snapshot of your external DB. Enablement of this feature is recommended for Production.")
