...
## Tower config files
## ------------------------------------------------------------------------------------
data "template_file" "tower_env" {
  template = templatefile("assets/src/tower_config/tower.env.tpl",
    {
...
data "template_file" "tower_yml" {
  # This bit of craziness is due to TF and then Bash interpolation.
  # https://stackoverflow.com/questions/66953938/escaping-dollar-sign-in-terraform
  template = templatefile("assets/src/tower_config/tower.yml.tpl",
    {
...
data "template_file" "tower_sql" {
  template = templatefile("assets/src/tower_config/tower.sql.tpl",
    {
...
data "template_file" "groundswell_sql" {
  template = templatefile("assets/src/groundswell_config/groundswell.sql.tpl",
    {
      db_tower_user                   = local.tower_secrets["TOWER_DB_USER"]["value"]
      db_tower_password               = local.tower_secrets["TOWER_DB_PASSWORD"]["value"]
      db_database_name                = var.db_database_name
...
data "template_file" "groundswell_env" {
  template = templatefile("assets/src/groundswell_config/groundswell.env.tpl",
    {
      db_tower_user                   = local.tower_secrets["TOWER_DB_USER"]["value"]
      db_tower_password               = local.tower_secrets["TOWER_DB_PASSWORD"]["value"]
      db_database_name                = var.db_database_name
...
data "template_file" "docker_compose" {
  template = templatefile("assets/src/docker_compose/docker-compose.yml.tpl",
    {
...
## ------------------------------------------------------------------------------------
data "template_file" "seqerakit_yml" {
  template = templatefile( "assets/src/seqerakit/setup.yml.tpl",
    {
...
## ------------------------------------------------------------------------------------
data "template_file" "aws_batch_manual" {
  template = templatefile("assets/src/seqerakit/compute-envs/aws_batch_manual.yml.tpl",
    {
...
data "template_file" "aws_batch_forge" {
  template = templatefile("assets/src/seqerakit/compute-envs/aws_batch_forge.yml.tpl",
  {
      aws_region                    = var.seqerakit_compute_env_region,
...
## ------------------------------------------------------------------------------------
data "template_file" "cleanse_and_configure_host" {
...
## Ansible
## ------------------------------------------------------------------------------------
data "template_file" "ansible_02_update_file_configurations" {
  template = templatefile("assets/src/ansible/02_update_file_configurations.yml.tpl",
    {
      app_name                          = var.app_name
      db_tower_user                     = local.tower_secrets["TOWER_DB_USER"]["value"]
      db_tower_password                 = local.tower_secrets["TOWER_DB_PASSWORD"]["value"]
...
data "template_file" "ansible_03_pull_containers_and_run_tower" {
  template = templatefile("assets/src/ansible/03_pull_containers_and_run_tower.yml.tpl",
    {
      app_name                          = var.app_name
    }
  )
...
data "template_file" "ansible_05_run_seqerakit" {
  template = templatefile("assets/src/ansible/05_run_seqerakit.yml.tpl",
    {
      app_name                          = var.app_name
    }
  )
...
data "template_file" "ssh_config" {
  template = templatefile("assets/src/ssh/ssh_config.tpl",
    {
