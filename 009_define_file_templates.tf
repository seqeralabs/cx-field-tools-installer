  ...
  docker_compose = templatefile("assets/src/docker_compose/docker-compose.yml.tpl",
    {
      ...
      updated_redis_version = (
        length(regexall("^v24\\.2\\.[0-9]", var.tower_container_version)) > 0 ||
        length(regexall("^v25\\.1\\.[0-9]", var.tower_container_version)) > 0
      )
      studio_uses_distroless = local.studio_uses_distroless,
  ...
  aws_batch_manual = templatefile("assets/src/seqerakit/compute-envs/aws_batch_manual.yml.tpl",
    {
      ...
      use_fusion_v2    = var.seqerakit_aws_use_fusion_v2 ? "True" : "False",
      use_wave         = var.seqerakit_aws_use_fusion_v2 ? "True" : "False",
      use_fast_storage = var.seqerakit_aws_use_fusion_v2 ? "True" : "False"
    }
  )
}
